<template>
  ...
  >
    <div v-if="$slots.before" ref="before" class="vue-recycle-scroller__slot">
      <slot name="before" />
    </div>
          ...
          ready
            ? {
                transform: `translate${direction === 'vertical' ? 'Y' : 'X'}(${
                  view.position
                }px) translate${direction === 'vertical' ? 'X' : 'Y'}(${view.offset}px)`,
                width: gridItems
    ...
    <div v-if="$slots.after" ref="after" class="vue-recycle-scroller__slot">
      <slot name="after" />
    </div>

    <ResizeObserver style="height: 1px !important;overflow: hidden" @notify="handleResize" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateVisibleItems(checkItem, checkPositionDiff = false) {
      ...
      let startIndex, mendIndex
      let totalSize
      let visibleStartIndex, visibleEndIndex

      if (!count) {
        startIndex = mendIndex = visibleStartIndex = visibleEndIndex = totalSize = 0
      } else if (this.$_prerender) {
        startIndex = visibleStartIndex = 0
        mendIndex = visibleEndIndex = Math.min(this.prerender, items.length)
        totalSize = null
      } else {
        ...
        if (itemSize === null) {
          ...
          // Searching for mendIndex
          for (
            mendIndex = i;
            mendIndex < count && sizes[mendIndex].accumulator < scroll.end;
            mendIndex++
          );
          if (mendIndex === -1) {
            mendIndex = items.length - 1
          } else {
            mendIndex++
            // Bounds
            mendIndex > count && (mendIndex = count)
          }
          ...
          startIndex -= remainer
          mendIndex = Math.ceil((scroll.end / itemSize) * gridItems)
      ...
      if (mendIndex - startIndex > config.itemsLimit) {
      ...
      const continuous = startIndex <= this.$_endIndex && mendIndex >= this.$_startIndex
      ...
      if (this.$_continuous !== continuous) {
        ...
        for (let i = 0, l = pool.length; i < l; i++) {
          ...
          if (view.nr.used) {
            ...
            if (view.nr.index === -1 || view.nr.index < startIndex || view.nr.index >= mendIndex) {
              this.unuseView(view)
            }
          }
        }
      ...
      let v
      for (let i = startIndex; i < mendIndex; i++) {
        item = items[i]
      ...
      this.$_endIndex = mendIndex

      if (this.emitUpdate)
        this.$emit('update', startIndex, mendIndex, visibleStartIndex, visibleEndIndex)
