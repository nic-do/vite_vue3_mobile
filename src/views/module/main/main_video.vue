...
const mixerOptions = {
  ...
  options: [
    'camera-screen',
    'multiple-cameras-default',
    'multiple-cameras-customized',
    'microphone-mp3'
  ]
}
mixerOptions.value = mixerOptions.options[1]
function getStream() {
  button.value.disabled = true
  if (videoPreview.value && !videoPreview.value.paused && videoPreview.value.srcObject) {
    videoPreview.value.stop()
  }
    ...
    <template v-slot:body>
      <div>
        ...
        <section class="experiment" style="text-align: center">
          <button ref="button" @click="getStream" id="btn-get-mixed-stream">
            Get Mixed Stream
          </button>
