          ...
          >
            ...
            >
              <span style="padding-bottom: 5px">
                {{scope.index+' '+ scope.data.title }}只在van-image中使用 --aspect-ratio
              </span>
...
<script>
import { ref, getCurrentInstance, computed, provide, inject, nextTick } from 'vue'
  ...
  setup(props, context) {
    ...
    const navDef = reactive({
      nav: PageNavDef.config({
        ...
        rightText: '关闭data',
        clickRight: function () {
          router.goBack(-1, { data: 'backfrom list-scroller 222' })
          return false
        }
      })
    })
  ...
  data() {
    return {
      ...
      config: {
        mode: 'scroller',
        rowHeight: 54,
        heigthDependencies: ['message'] //scroller用，message按实际修改，影响高度计算的字段
      }
    }
  },
  created() {},
