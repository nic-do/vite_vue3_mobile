...
export class MiniMap {
  ...
  add() {
    ...
    this._miniMapDomEl = markRaw(mapRenderer.domElement)
    document.body.appendChild(mapRenderer.domElement)

    // 初始化小地图相机
    const mapCamera = this.com.track(
  ...
  updateCamera() {
    ...
    if (this._followTarget) {
      ...
      this._miniMapCamera.position.set(targetPos.x, targetPos.y + 10, targetPos.z)
      this._miniMapCamera.lookAt(targetPos.x, 3, targetPos.z)
    } else {
      this._miniMapCamera.position.set(0, 10, 0)
      this._miniMapCamera.lookAt(0, 3, 0)
    }
  }
  ...
  change(mapSize, mapRenderSize) {
    if (mapRenderSize > 0) {
      this.mapRenderSize = mapRenderSize

      this._miniMapRenderer.setSize(mapRenderSize, mapRenderSize)
    }
    if (mapSize > 0) {
      this.mapSize = mapSize
      ;(this._miniMapCamera.left = -mapSize / 2), (this._miniMapCamera.right = mapSize / 2)
      this._miniMapCamera.top = mapSize / 2
      this._miniMapCamera.bottom = -mapSize / 2
