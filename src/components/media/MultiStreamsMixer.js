...
const MultiStreamsMixer = function (arrayOfMediaStreams, elementClass) {
  ...
  function drawVideosToCanvas() {
    ...
    if (fullcanvas) {
      // if (fullcanvas.videoWidth!=undefined){
      //   canvas.width = fullcanvas.videoWidth
      //   canvas.height = fullcanvas.videoHeight
      // }else
      {
  ...
  function drawImage(video, idx) {
    ...
    if (typeof video.stream.onBeforeRender === 'function') {
      video.stream.onBeforeRender(context, x, y, width, height, idx)
    }
    // let fontCameraNeedRotate = true
    // if (fontCameraNeedRotate) {
    //   //镜像翻转图片
  ...
  let refresh_el = null
  let refresh_el_id = 'refresh_el'
  function getVideo(stream, idx) {
    let el = document.getElementById(refresh_el_id)
    if (!el&&ismobileSafari()) {
      //手机端 显示的画面不刷新，需要额外挂一个 div去触发 界面刷新。见上面定时刷新
      //这里对div 刷的是时间。透明掉不影响
    ...
    let flag = true
    if (typeof stream.onVideoPlay === 'function') {
      if (ismobileSafari()) {
        //ios 不能直接调用play
      ...
      flag = stream.onVideoPlay(video, idx)
    }
    if (flag) {
      //ios 不能直接调用play
      video
        .play()
        .then(function () {
          // console.log('---video.play--', 'start-1')
        })
        .catch(function (error) {
          ...
          if (ismobileSafari()) {
            ...
            // console.log('---video.play--', 'start-0')
            let res = confirm('允许播放')
            if (res) {
              video
                .play()
                .then(function () {
                  // console.log('---video.play--', 'start-11')
                })
                .catch(function (error) {
                  // console.log('---video.play-11-', error)
                })
            }
          }
        })
